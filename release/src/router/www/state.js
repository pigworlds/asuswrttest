...
if(realip_support){
	if(active_wan_unit == "0"){
		var realip_state = '<% nvram_get("wan0_realip_state"); %>';  //0: init/no act  1: can't get external IP  2: get external IP
		var realip_ip = '<% nvram_get("wan0_realip_ip"); %>'; 
		var external_ip = (realip_ip == '<% nvram_get("wan0_ipaddr"); %>')? 1:0;
	}
	else if(active_wan_unit == "1"){
		var realip_state = '<% nvram_get("wan1_realip_state"); %>';  //0: init/no act  1: can't get external IP  2: get external IP
		var realip_ip = '<% nvram_get("wan1_realip_ip"); %>'; 
		var external_ip = (realip_ip == '<% nvram_get("wan1_ipaddr"); %>')? 1:0;
	}
}
...
tabtitle[0] = new Array("", "<#menu5_1_1#>", "<#menu5_1_2#>", "WDS", "<#menu5_1_4#>", "<#menu5_1_5#>", "<#menu5_1_6#>");
...
tabtitle[4] = new Array("", "<#UPnPMediaServer#>", "<#menu5_4_1#>", "<#menu5_4_2#>");
tabtitle[5] = new Array("", "IPv6");
tabtitle[6] = new Array("", "<#BOP_isp_heart_item#>", "VPN Client", "TOR");	//, "VPN Client Advanced"
...
if(bwdpi_support){
	if(based_modelid == "RT-AC3200"){
		...
		tabtitle[12] = new Array("", "<#AiProtection_Home#>", "<#Parental_Control#>", "Ad Blocking");
		tabtitle[13] = new Array("", "Time Limits", "<#AiProtection_filter#>");
		tabtitle[14] = new Array("", "Statistic", "Traffic Monitor");
	}
	else{
		if(spectrum_support)
			tabtitle[11] = new Array("", "<#Bandwidth_monitor#>", "QoS", "<#Adaptive_History#>", "<table style='margin-top:-10px \\9;'><tr><td><img src='/images/ROG_Logo.png' style='border:0px;width:32px;'></td><td>ROG First</td></tr></table>", "<#traffic_monitor#>", "Spectrum");
		else
...
tablink[4] = new Array("", "mediaserver.asp", "Advanced_AiDisk_samba.asp", "Advanced_AiDisk_ftp.asp");
tablink[5] = new Array("", "Advanced_IPv6_Content.asp");
tablink[6] = new Array("", "Advanced_VPN_PPTP.asp", "Advanced_VPNClient_Content.asp", "Advanced_TOR_Content.asp");
...
if(bwdpi_support){
	if(based_modelid == "RT-AC3200"){
		tablink[11] = new Array("", "AdaptiveQoS_Bandwidth_Monitor.asp", "QoS_EZQoS.asp", "AdaptiveQoS_WebHistory.asp", "AdaptiveQoS_ROG.asp", "Main_Spectrum_Content.asp", "Advanced_QOSUserPrio_Content.asp", "Advanced_QOSUserRules_Content.asp", "AdaptiveQoS_Adaptive.asp");
		tablink[12] = new Array("", "AiProtection_HomeProtection.asp", "AiProtection_WebProtector.asp", "AiProtection_AdBlock.asp");
	...
	tablink[11] = new Array("", "QoS_EZQoS.asp", "Main_TrafficMonitor_realtime.asp", "AdaptiveQoS_ROG.asp", "Main_Spectrum_Content.asp", "Main_TrafficMonitor_last24.asp", "Main_TrafficMonitor_daily.asp", "Advanced_QOSUserPrio_Content.asp", "Advanced_QOSUserRules_Content.asp");
...
if(bwdpi_support){
	if(based_modelid == "RT-AC3200"){
		...
		menuL1_link = new Array("", "index.asp", "Guest_network.asp", "AiProtection_HomeSecurity.asp", "AdaptiveQoS_Bandwidth_Monitor.asp", "TrafficAnalyzer_Statistic.asp", "APP_Installation.asp", "cloud_main.asp", "");
	}
	else{
		menuL1_title = new Array("", "<#menu1#>", "<#Guest_Network#>", "AiProtection", "<#Adaptive_QoS#>",  "<#Menu_usb_application#>", "AiCloud 2.0", "<#menu5#>");
		menuL1_link = new Array("", "index.asp", "Guest_network.asp", "AiProtection_HomeSecurity.asp", "AdaptiveQoS_Bandwidth_Monitor.asp",  "APP_Installation.asp", "cloud_main.asp", "");
	}
}
else{
	menuL1_title = new Array("", "<#menu1#>", "<#Guest_Network#>", "<#Menu_TrafficManager#>", "<#Parental_Control#>", "<#Menu_usb_application#>", "AiCloud 2.0", "<#menu5#>");
...
function remove_url(){
	...
	if(WebDav_support) {
		 tabtitle[4][2] = "<#menu5_4_1#> / <#Cloud_Disk#>";
	}
	...
	if(!pptpd_support && !openvpnd_support){
		...
		var VPNServer_mode = '<% nvram_get("VPNServer_mode"); %>';
		var VPNServer_enable = '<% nvram_get("VPNServer_enable"); %>';
		if((!pptpd_support && openvpnd_support) || (VPNServer_mode == "openvpn" && VPNServer_enable == "1")){
			tablink[6][1] = "Advanced_VPN_OpenVPN.asp";
			menuL2_link[7] = "Advanced_VPN_OpenVPN.asp";
		}
	}
...
function show_menu(){
	...
	if(bwdpi_support && based_modelid == "RT-AC3200"){
	...
	if(current_url.indexOf("Advanced_VPN") == 0){
		...
		L2 = traffic_L2_dx;
		L3 = 1;
		if(current_url.indexOf("Advanced_VPNClient_Content.asp") == 0 && (pptpd_support || openvpnd_support)) {
			L3 = 2;
		}
	}	
	...
	if(gobi_support && (usb_index != -1) && (sim_state != "")){//sim record full warning
	...
	if(realip_support && !external_ip){
		...
		notification.desc[14] = "The WAN IP is not the external IP. External IP-based services won't work.";
  ...
  if(swpjverno == ''){
	...
	if (!dsl_support) {
		if(sw_mode == "1")  // Show operation mode in banner, Viz 2011.11
			...
			document.getElementById("sw_mode_span").innerHTML = "Media bridge";
		else
...
var isNewFW = function(FWVer){
	...
	if (odmpid == "TM-AC1900" && (bl_version == "2.1.2.2" || bl_version == "2.1.2.5"))
		return true;
