...
function openvpn_clientlist(){
	...
	code +='</table>';
	$("openvpn_clientlist_Block").innerHTML = code;
}
...
function del_openvpnRow(r){
	...
	$('openvpn_clientlist_table').deleteRow(i);
   ...
   if(check_openvpn_conflict()){
	...
	for(i=0; i<client_num; i++){
		...
		for(j=0; j<item_num-1; j++){
			...
			if (j == 3)
				tmp_value += ($('openvpn_clientlist_table').rows[i].cells[j].innerHTML == "Yes" ? 1 : 0);
			else
				tmp_value += $('openvpn_clientlist_table').rows[i].cells[j].innerHTML;
                ...
                <tr>
					...
					<table width="100%" id="page1_static" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable">
						<tr>
							...
							<td>
								<textarea rows="8" class="textarea_ssh_table" id="edit_vpn_crt_server1_static" name="edit_vpn_crt_server1_static" cols="65" maxlength="2999"><% nvram_clean_get("vpn_crt_server1_static"); %></textarea>
							</td>
						</tr>
					</table>
						<div style="margin-top:5px;width:100%;text-align:center;">
						...
						<table width="760px" border="0" cellpadding="4" cellspacing="0" class="FormTitle">
							<tbody>
     		   				...
     		   				<select onchange="switchPage(this.options[this.selectedIndex].value)" class="input_option">
											<option value="1"><#menu5_1_1#></option>
											<option value="2" selected><#menu5#></option>									
									</select>
								</div>
                		...
                		<tr bgcolor="#4D595D">
                			<td valign="top">
								...
								<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3"  class="FormTable">
									...
									<tr id="client_unit" style="display:none;">
									...
									<tr id="service_enable_button" style="display:none;">
										<th>Service state</th>
										<td>
			        			...
			        			<td>
			        				<select name="vpn_server_crypt" class="input_option" onclick="update_visibility();">
											...
											<span id="server_static_crypto_text" onclick="set_Keys('secret');" style="text-decoration:underline;cursor:pointer;"><#vpn_openvpn_ModifyKeys#></span>
