    ...
    struct ol_txrx_stats_req *req)
{
    ...
    adf_os_print("Tx pkts completed\t:  %u\n", stats->tx.delivered.pkts);
    adf_os_print("Tx bytes completed\t:  %u\n", stats->tx.delivered.bytes);
    adf_os_print("Tx pkts from stack\t:  %u\n", stats->tx.from_stack.pkts);

    adf_os_print("\n");
    adf_os_print("++++++++++ HOST RX STATISTICS +++++++++++\n");
    adf_os_print("Rx pkts completed\t:  %u\n", stats->rx.delivered.pkts);
    adf_os_print("Rx bytes completed\t:  %u\n", stats->rx.delivered.bytes);
#if QCA_OL_SMP_STATS
    adf_os_print("\n");
...
ol_txrx_stats_display(ol_txrx_pdev_handle pdev)
{
    ...
    adf_os_print(
        "  rx: %lld ppdus, %lld mpdus, %u msdus, %u bytes, %lld errs\n",
