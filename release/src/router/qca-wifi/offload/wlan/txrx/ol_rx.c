    ...
    )
{
    ...
    if (adf_nbuf_len(msdu) < sizeof(struct ieee80211_frame)) {
#if RX_DEBUG
        uint8_t *peer_mac = &(htt_pdev->g_peer_mac[0]);
        printk("%s: msdu len: %d, peer_id: %d, num mpdu: %d, tid: %d, status: 0x%x, assertFlag: 0x%x,"
                " Mac: %x:%x:%x:%x:%x:%x\n",
                __func__, adf_nbuf_len(msdu), htt_pdev->g_peer_id, htt_pdev->g_num_mpdus,
                htt_pdev->g_tid, htt_pdev->g_htt_status, htt_pdev->htt_rx_donebit_assertflag,
                peer_mac[0], peer_mac[1], peer_mac[2], peer_mac[3], peer_mac[4], peer_mac[5]);
        /* dump descriptor */
        dump_htt_rx_desc(rx_mpdu_desc);
        /* dump packet */
        dump_pkt(msdu, adf_nbuf_len(msdu));
#endif
        return;
    }
    ...
    for (mpdu_range = 0; mpdu_range < num_mpdu_ranges; mpdu_range++) {
        ...
        if ((status == htt_rx_status_ok) && peer) {
            ...
            for (i = 0; i < num_mpdus; i++) {
                ...
                /*Check to discard Phy Error packets*/
                if (!(skip_monitor || (rx_desc->ppdu_end.phy_err))) {
...
extern struct ieee80211vap *vv_first_vap;
extern struct ol_ath_softc_net80211 *vv_first_vap_scn;
...
#endif
    vdev->osif_rx(vdev->osif_vdev, deliver_list_head);
    if (vv_first_vap_scn && !vv_first_vap_scn->scn_stats.ap_stats_tx_cal_enable) {
	struct ieee80211_mac_stats *mac_stats=&vv_first_vap->iv_unicast_stats;
#if 0
                mac_stats->ims_rx_data_packets++;
                mac_stats->ims_rx_data_bytes += byte_cnt;
                mac_stats->ims_rx_datapyld_bytes += (byte_cnt - ETHERNET_HDR_LEN);
#endif
                mac_stats->ims_rx_packets++;
                mac_stats->ims_rx_bytes += byte_cnt;
    }
#if OL_ATH_SUPPORT_LED
    {
        struct ol_ath_softc_net80211 *scn = (struct ol_ath_softc_net80211 *)pdev->ctrl_pdev;
