...
#define LTEU_SUPPORT1    0x2
    if (!strcmp("wifi0", dev->name))
        scn->lteu_support = (lteu_support & LTEU_SUPPORT0) ? 1 : 0;
    else if (!strcmp("wifi1", dev->name))
...
#ifdef LOAD_ARRAY_FW
#if 1 // for ASUS RT-AC55U, force use eeprom in NAND flash
...
#if 1 //def  ATH_CAL_NAND_FLASH
        // for ASUS RT-AC55U, force use eeprom in NAND flash
...
#if 1
            // for ASUS RT-AC55U, force use eeprom in NAND flash
            ret_val = ath_nand_local_read("Factory", 0x5000, QC98XX_EEPROM_SIZE_LARGEST, &ret_len, ptr);
#else
